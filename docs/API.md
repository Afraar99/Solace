...
Solace supports **deep linking** for external apps, browsers, or command-line tools. Right now it mainly supports opening screens; deeper third-party actions are planned.

> Package / scheme: `com.mindful.android` (unchanged from upstream Mindful for compatibility).
...
> Navigates the user to a specific screen with the provided parameters.
...
* ### `/appDashboard`
  ...
  - **Parameters:**
    ...
    - `usageType` (Optional)
      ...
      - `1` → Network usage
    - `day` (Optional) — date as `yyyy-mm-dd` (defaults to today)
...
> Navigate to a specific screen and perform some action automatically.

---

## Testing

Please ensure Solace is installed before testing.
