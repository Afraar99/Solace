...
/**
 * VPN service that blocks internet for selected apps (kill switch).
 * NSFW website blocking uses Accessibility — not this VPN.
 */
        ...
        get() = Runnable {
            try {
                DatagramChannel.open().use { tunnel ->
                    check(this@MindfulVpnService.protect(tunnel.socket())) {
